...
def preprocess_parent(dir,tokenizer,k=0,side=1,merge_type=0,one_ans=0):
    print('Start reading and parsing csv',file=sys.stderr)
    tags=['調達年度','都道府県','入札件名','施設名','需要場所(住所)','調達開始日','調達終了日','公告日','仕様書交付期限','質問票締切日時','資格申請締切日時','入札書締切日時',\
        '開札日時','質問箇所所属/担当者','質問箇所TEL/FAX','資格申請送付先','資格申請送付先部署/担当者名','入札書送付先','入札書送付先部署/担当者名','開札場所']
    sep_token='[SEP]'
    ...
    for a in res:
        ...
        for i in ks:
            ...
            nxt.append((tree[i],par[i],parind[i],partxt[i]))
    print('Start creating dataset',file=sys.stderr)
    ret=[]
    for tag in tqdm(tags):
        for a,par,parind,partxt in nxt:
            bnd=[]
            toklen=[]
...
#preprocess_parent('train/ca_data/*',AutoTokenizer.from_pretrained("bert-base-multilingual-cased"),0,1,0,1)
